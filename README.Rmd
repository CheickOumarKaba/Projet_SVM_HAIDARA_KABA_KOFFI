---
title: "Implémentation des SVM et performances : Cas de la detection de fraude sur les cartes de crédits"
...
load ("C:/Users/cheic/Documents/GitHub/daishi/newdata.Rdata")
load("C:/Users/cheic/Documents/GitHub/daishi/base.Rdata")
library(ROCR)
library(rmarkdown)
library(e1071)
library(ISLR)
...
Cette étude s'inscrit dans le cadre de l'implémentation et les performances des modèles SVM (Machines à vecteurs de support) dans le cadre de la détection de fraude sur les cartes de crédit.  
Pour se faire, nous disposons d'une base de données disponibe sur le site Kaggle.  
...
paged_table(newdata,options = list(rows.print=5))
```  
...
*Perc.over* : controle la quantité du sur-écahantillonnage de la classe minoritaire.   
*Perc.under* : controle la quantité du sous-échantillonnage de la classe majoritaire.  
*k* : Nombre indiquant le nombre de voisins les plus proches utilisés pour générer les nouveaux exemples de la classe minoriataire.  
...
Nous créons ainsi une nouvelle base de données contenant **`r nrow(newdata)`** lignes qui nous servira pour la modélisation. Cette nouvelle base admet la repartition suivante pour la variable d'intéret "Class" :  
...
Les SVM (Machines à Vecteur de Support, ou encore séparateur à vaste marge) est une technique d'apprentissage supervisé qui s'inscrit dans le cadre de la résolution des problèmes d'apprentissage. Cette méthode repose sur deux notions clés : les notions de marges maximales et de fonction de noyau (kernel).   
...
###  __2) <u>Cass pratique : application des SVM au cas de la détection de fraude sur les cartes de crédit </u>__  
  
####  __<u>Cas échantillon linéairement/non linéairement séparable</u>__  
  
Nous réalisons une représentation de quelques variables explicatives en fonction des modalités de la variable d'intérêt "Class" de la base pour avoir une idée du cas d'échantillon auquel on est confronté
```{r, echo= FALSE}
attach(newdata)
par(mfrow=c(3,3))
...
Nous choisissons d'utiliser 70% des observations de l'écantillon initiale comme échantillon d'apprentissage (newdata.train) et les 30% restants comme notre échantillon test (newdata.test).  
```{r, echo=FALSE}
...
la fonction tune(svm...) du package "e1071" de R nous donne ma possiblité de déterminer automatiquement par validation croisée la fonction kernel et les valeurs des hyper-paramètres optimaux associés de façon automatique.  
...
![40% center](C:/Users/cheic/Documents/GitHub/daishi/images/tune_svm.png)  
...
Les SVM, méthodes d'apprentissage supervisée peuvent se montrer très utile pour plusieurs raisons :  
...
• Robuste par rapport aux problèmes de multicolinéarités  
Cependant, cette technique presénte également quelques inconvénients, à savoir :  
...
• Problème lorsque l'on est face à un cas d'échantillon déséquilibré  
• Difficulté d’interprétations (exemple : pertinence des variables)  
  
Nous avons construit un démonstrateur en ligne avec R shiny qui vous permettra de vous rendre compte par vous même de la facillité d'emploi des SVM, de visualiser ses performances en jouant sur le choix des kernels, des hyper-paramètres dans le cadre de la detection de fraude sur les carte de crédits. Ce démonstrateur vous permettra par aillleurs de Comparez les performances du SVM à celle de la régression logistique.  
Pour cela, vous devez au préalable télécharger la base de donnée "newdata.csv" qui désigne la base disponible sur Kaggle à laquelle nous avons apporté quelques modifications.  
Cette base est disponible en téléchargement [__***ici***__](https://github.com/CheickOumarKaba/daishi/blob/master/newdata.csv) 
